...
```
Vemos que la máxima correlacion se obtiene para un desfase de 18 horas. 
```{r}
plot(r$acf, x= r$lag, 
     xlim = c(-72,72),
     xlab = "lag",
     ylab = "cor",
     main = "Cross-correlation punto 23")
```
...
Se puede ver que aumentando SMA de la lluvia, la predicción de la variación de nivel es muy buena... imagino que será por cositas de inercia.  A continuación... hacemos exactamente lo mismo pero metiendo 
...
predict_1$SMA_prep<- SMA(predict_1$prep_hourly,i)
predict_1$SMA_difnivel<- SMA(predict_1$dif_nivel, 36)
predict_1$SMA_prep_lag1<- lag(predict_1$nivel,24)
...
train$SMA_prep<- SMA(train$prep_hourly,i)
train$SMA_difnivel<- SMA(train$dif_nivel, 36)
train$SMA_prep_lag1<- lag(train$nivel,24)
train<- train[complete.cases(train),]
...
partiendo de estas conclusiones a continuación realizo las mismas gráficas pero variando el valor de nivel que entra a nuestro modelo... nivel -1, nivel -2 nivel -3 etc. 
...
DHI$turbinado<- prueba2$`Turbinado_BCE[m³/s]`+prueba2$`Turbinado[m³/s]`
...
DHI$turbinado_SMA1<- SMA(DHI$turbinado, 48)
