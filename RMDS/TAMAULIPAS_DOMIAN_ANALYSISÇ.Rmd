...
aggregate(TABLA_ACCURACY[, 1:6], list(MODELO= TABLA_ACCURACY$MODELO), FUN = function(x) mean(as.numeric(as.character(x))))


aggregate(TABLA_ACCURACY[,c( 'CORR')], 
          list(MODELO= TABLA_ACCURACY$MODELO), FUN = function(x) which.max(x) )

TABLA_MAX_COR<- TABLA_ACCURACY %>% group_by(MODELO) %>% group_split() %>% lapply(function(x) x[which.max(x$CORR), ]) %>% bind_rows()



D01 %>% filter(LON== TABLA_MAX_COR %>% .[which(.$MODELO=='D01'), 'LON'] %>% as.numeric(),
               D01$LAT==TABLA_MAX_COR %>% .[which(.$MODELO=='D01'), 'LAT'] %>% as.numeric())
