...
For the elaboration of this predicctive model we will use ARW-WRF model rain output. It's remarkable that we are getting very good correlations between WRF model data and the historical data.  Maybe the best noticce is that the best correlation is for the long-term cumulative rain results. 

<br />
<br />

All the procces is being carried out using R and Rstudio tools, because of it's verstatility and good performance for machine learning purposes. The first predictive models that we have elaborate are being developed using common regression techniques, but the are getting good performance only for the next 3 days. For getting a good long-term prediction, more sofisticated methods should be implemented. 

<br />
<br />

Nowadays, we are calculating the best predictive model for achieved a good performance for a long-term prediction. For this purpose we are using the R "caret" package that is and recognized tool for machine learning using R lenguaje. For getting the best model using this tool we are using cross-validation techniques for more that 45 algorithms, integrated in caret package. After the best algorithms are being selected, another procces for adjusting the parameters will be carried out. 
...
This figure shows that WRF model is even better in the daily rainfall stimation and also in the total cumulative rain. Also, this figure shows that the streamflow is highly correlated whith the rainfall with some lag. 
<br />
<br />
...
Next figures shows that, because of the high correlation between WRF rainfall stimation and streamflow, we are able to make an accurate short term prediction using simple regression techniques. for the estimation of the level variation of the dam and also the streamflow reaching the dam. 
<br />
<br />
...
<br />
<br />
In the next table all the complex algorithms that are going to be tested using R "caret" package are shown. 
...
<br />
<br />
All the tests needed are not actually made but, this is one of the first results using Adaptive neuro fuzzy inference system (ANFIS).
<br />
<br />
```{r }
library(caret)
...
modelo_knn1<- readRDS(here::here('Data/Parques/Belesar/Modelos/ANFIS_1.RDS'))

modelo_knn2<- readRDS(here::here('Data/Parques/Belesar/Modelos/ANFIS_2.RDS'))
...
gg_graph<- ggplot(data = prediccion)+
  ...
  xlab("Date")+
  ggtitle("Level variation prediction \n using ANFIS algorithm")+
