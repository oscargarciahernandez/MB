...
<br />
<br />
All the tests needed are not actually made but, this is one of the first results using Adaptive Neuro Fuzzy Inference System (ANFIS). We have take into account that this preliminary results are not very good because of **it's important to adjust some parameters meticulously and also test with different input variables before get the final predictivie model.**
<br />
<br />
```{r }
library(caret)
...
gg_graph<- ggplot(data = prediccion)+
  ...
  labs(subtitle = "Actual dam level variation (black) \nTwo different predictive models using ANFIS (green, blue)")+
