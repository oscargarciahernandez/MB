...
Este documento es para incluir el proceso "final" del estudio enfocado al predictive model de Belesar. 
...
# Comparacion WRF-Observada sin modificar
...
Tal y como hemos visto con weka y mirando algunos papers por ahí. Vamos a probar en primera instancia: 
...
Vamos ha hacer dos rondas. Una primera solamente teniendo en cuenta la lluvia WRF (modificada con SMA)
```{r include=FALSE}
library(caret)
library(doMC)
...
#################### Los valores de i y J son: cositas con las que se puede juga... son el valor del SMA sobre la lluvia (i) y el valor del retraso en horas del dato de lluvia -x
i<- 48
j<- 54 
...
library(RWeka)
library(rJava)
modelo_knn<-  readRDS(here::here('Data/Parques/Belesar/Modelos/M5_1.RDS'))
modelo_knn1<-  readRDS(here::here('Data/Parques/Belesar/Modelos/M5_2.RDS'))
modelo_knn2<- readRDS(here::here('Data/Parques/Belesar/Modelos/M5_3.RDS'))
...
```
```{r}
tabla_svm_models<- list.files(here::here('Data/Parques/Belesar/Modelos/'), full.names = T) %>% .[str_detect(., ".RDS")] %>% .[str_detect(., "svm")] %>% matrix(., ncol = 3, byrow = T)
colnames(tabla_svm_models)<- c("m1", "m2", "m3")
tabla_svm_models<- as.data.frame(tabla_svm_models)
```

```{r}
for (i in 1:length(tabla_svm_models$m1)) {
  modelo_knn<-  readRDS(as.character(tabla_svm_models[i,1]))
modelo_knn1<-  readRDS(as.character(tabla_svm_models[i,2]))
modelo_knn2<- readRDS(as.character(tabla_svm_models[i,3]))
  

uncorrected<-prediccion$dif_nivel
  
titulo<- as.character(tabla_svm_models$m1[i]) %>% str_split(.,"/") %>% .[[1]] %>% .[str_detect(.,".RDS")] %>% str_split(.,"_") %>% .[[1]] %>% .[1]
  ...
  gg_graph<- ggplot(data = prediccion)+
    ...
    xlab(paste0(range(prediccion$Date)))+
    ylab ("nivel [msnm]")+
    ggtitle(paste0("Predicción de nivel \n solo con lluvia WRF  \n  con un lag de  ", j, " horas. Método ", titulo))+
    ylim( c(-0.1,0.1))+
    geom_line(aes(y=SMA_difnivel,
                  x=Date), 
              col="black", lty=2)+
...
```
A continuacion probamos cambiando el parámetro Cost...
```{r}
tabla_svm_models<- list.files(here::here('Data/Parques/Belesar/Modelos/'), full.names = T) %>% .[str_detect(., ".RDS")] %>% .[str_detect(., "svm")] %>% .[str_detect(.,"tune")]%>% matrix(., ncol = 3, byrow = T)
colnames(tabla_svm_models)<- c("m1", "m2", "m3")
tabla_svm_models<- as.data.frame(tabla_svm_models)
```
...
Vemos que variando el parámetro C (cost) no se consigue aumentar la calídad de la gráfica.
Ahora vamos a jugar con SVM y variando los parámetros i y j.... SMA sobre los datos de lluvia WRF y retraso... sobre la lluvia WRF. 


```{r}
# i= SMA lluvia
# j= retraso WRF_SMA 
for(i in seq(24, 96, by=6)){
  
  for (j in seq(24, 72, by=6)) {
...
modelo_knn<-tryCatch({train(SMA_difnivel ~ SMA_prep_lag1, data = entrenamiento,
  ...
  cor1<- round(cor(prediccion$SMA_difnivel,predict(modelo_knn,newdata = prediccion)), digits = 3)
  cor2<- round(cor(prediccion$SMA_difnivel,predict(modelo_knn1,newdata = prediccion)), digits = 3)
  cor3<- round(cor(prediccion$SMA_difnivel,predict(modelo_knn2,newdata = prediccion)), digits = 3)
  cormax<- max(cor1, cor2,cor3, na.rm = T)
  posmax<- which.max(c(cor1, cor2, cor3))
  ...
  gg_graph<- ggplot(data = prediccion)+
    ...
    xlab(paste0(range(prediccion$Date)))+
    ylab ("nivel [msnm]")+
    ggtitle(paste0("Predicción de nivel \n solo con lluvia WRF  \n  con un SMA",i, " y lag de  ", j, " horas"))+
    labs(subtitle = paste0("Cor = ", cormax, "_",posmax))+
    ylim( c(-0.1,0.1))+
    geom_line(aes(y=SMA_difnivel,
                  x=Date), 
              col="black", lty=2)+
  ...
  name_base<- "svmLinear"
  nombre1<- paste0(name_base,"_",i,"_",j, "_1.RDS")
  nombre2<- paste0(name_base,"_",i,"_",j, "_2.RDS")
  nombre3<- paste0(name_base,"_",i,"_",j, "_3.RDS")
  
  nombre_img<- paste0(name_base,"_",i,"_",j, ".PNG")
