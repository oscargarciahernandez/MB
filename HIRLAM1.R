...
HIRLAM_DATA<- read.csv(PATH_CERROBLANCO %>% list.files(full.names = TRUE) %>% .[2])
HIRLAM_DATA$DATE<- HIRLAM_DATA$DATE %>% ymd_hms()
x<- HIRLAM_DATA %>% group_by(LON,LAT) %>% group_split() %>% .[[1]]


x$T_MODEL<- x$DATE - as.difftime(x$FCST_TIME, units = 'hours')
x<- x %>% .[order(x$T_MODEL), ]
x2<- x %>% group_by(T_MODEL) %>% group_split() %>% .[[1]]

HIRLAM_COMPLETE<- data.frame(DATE= seq((x2$DATE %>% range())[1],
                                       (x2$DATE %>% range())[2],by = 'hour'))

HIRLAM_COMPLETE<- left_join(HIRLAM_COMPLETE, x2, by='DATE')

library(imputeTS)
HIRLAM_INTERPOLATED<- HIRLAM_COMPLETE %>% na.interpolation() 
HIRLAM_INTERPOLATED$T_MODEL<- HIRLAM_INTERPOLATED[!is.na(HIRLAM_INTERPOLATED$T_MODEL),"T_MODEL"] %>% .[1]


TABLA_INTERPOLATED<- HIRLAM_DATA %>% group_by(LON,LAT) %>% group_split() %>% 
  lapply(function(x){
    x$T_MODEL<- x$DATE - as.difftime(x$FCST_TIME, units = 'hours')
    x<- x %>% .[order(x$T_MODEL), ]
    HIRLAM_I<- x %>% group_by(T_MODEL) %>% group_split() %>% lapply(function(x2){
       HIRLAM_COMPLETE<- data.frame(DATE= seq((x2$DATE %>% range())[1],
                                              (x2$DATE %>% range())[2],by = 'hour'))
       ...
       return(HIRLAM_INTERPOLATED)
     }) %>% bind_rows()
    return(HIRLAM_I)
  })

TABLA_INTERPOLATED<- TABLA_INTERPOLATED %>% bind_rows()
...
INFO_CERROBLANCO<- INFO_PARQUES %>% filter(PARQUE=='P.E. Cerroblanco')

MERGE_CERROBLANCO<- left_join(TABLA_INTERPOLATED, INFO_CERROBLANCO, by= 'DATE')

MERGE_CERROBLANCO %>% .[complete.cases(.),] %>% 
  group_by(LAT.x, LON.x) %>% group_split() %>% 
  lapply(function(x){
    range(x$DATE)
    #cor(x$WS10, x$PRUDUCCION_MWH)
  })
MERGE_CERROBLANCO %>% colnames()

MERGE_CERROBLANCO %>% .[complete.cases(.),c("DATE","FCST_TIME","LON.x","LAT.x","WS10","WD10",
                                            "WS100","WD100","U10","V10","U100","V100","T_MODEL",
                                            "PRUDUCCION_MWH","DISPONIBILIDAD")] %>% write.csv(paste0(PATH_CERROBLANCO, 'HISTORICO_HIRLAM_INTERPOLATED_WITH_PRUDCTION.csv'))
