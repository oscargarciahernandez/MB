def _LecturaWRF_V10_VN5_VN7_VN9_T02_ARW(Direccion_ArchivosWRF,fechaInicio):
	...
	Nombre_Parque='ARW'
	Longitud_Parque=14.325
	Latitud_Parque=50.059
	Direccion_Resultados='D:\\Users\\EDUARDO\\Documents\\Eolocast\\WRF\\HISTORICO\\ARW\\Europa_d01'
	lenght=24
	...
	fP=open('%s\WRF_P_%s.txt'%(Direccion_Resultados,Nombre_Parque),'a')
	...
	fP=open('%s\WRF_P_%s.txt'%(Direccion_Resultados,Nombre_Parque),'r')
	...
	lT=fT.readlines()
	if len(lu)==len(lv) and len(lu)==len(lMV) and len(lu)==len(lT) and len(lu)==len(luN5) and len(lu)==len(luN7) and len(lu)==len(lvN5) and len(lu)==len(lvN7) and len(lu)==len(lMVN5) and len(lu)==len(lMVN7) and len(lu)==len(lP) and len(lu)==len(lvN9):
		...
		for l in range(lenght):
				...
				for i in range(1,(N_Lat-1)):
					...
					for j in range(1,(N_Lon-1)):
						if Lon[i,j-1]<Longitud_Parque and Lon[i,j+1]>=Longitud_Parque and Lat[i-1,j]<Latitud_Parque and Lat[i+1,j]>=Latitud_Parque:
							...
							break
					if stop==1:break
			u=data.variables['HGT'][0][:]
			uN5=data.variables['T02_MAX'][0][:]-273.15
			uN7=data.variables['PSFC'][0][:]/100
			P=(uN7*(1-((0.0065*u)/(uN5+0.0065*u+273.15)))**(-5.257))
			#uN5=data.variables['U'][0][5]
			#uN7=data.variables['U'][0][7]
			#P=data.variables['U'][0][9]
			...
			if len(lu)==0 and l==0:
				...
				for hm in range((m-36),((m+36)+1)):
					for hn in range((n-44),((n+44)+1)):
						fu.write('%1.4f\t'%(Lon[hm,hn]))
						fuN5.write('%1.4f\t'%(Lon[hm,hn]))
						fuN7.write('%1.4f\t'%(Lon[hm,hn]))
						fP.write('%1.4f\t'%(Lon[hm,hn]))
