...
nivel_actual<- pred1$`Nivel[msnm]`[abs(length(aport)-length(pred1$prep_hourly)-1):length(pred1$Date)]

data_predict_cut<- as.data.frame(cbind(aport,
                                       prep, 
                                       aport_actual, 
                                       nivel_actual))
...
aport_actual_test<- pred2$`Aportacion[m³/s]`[1:length(aport_test)]
